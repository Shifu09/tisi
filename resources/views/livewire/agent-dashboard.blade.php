...
new class extends Component {
    ...
    {
        $this->assignedTickets = auth()->user()->assignedTickets()
            ->with(['user', 'category'])
            ->latest()
            ->get();
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-lg shadow">
            <div class="px-6 py-4 border-b border-gray-200">
                <h3 class="text-lg font-medium text-gray-900">Mis Tickets Asignados ({{ $assignedTickets->count() }})</h3>
            </div>
                    ...
                    @foreach($assignedTickets as $ticket)
                        <div class="px-6 py-4">
                            <div class="flex items-center justify-between mb-2">
                                <a href="/tickets/{{ $ticket->id }}" class="text-sm font-medium text-indigo-600 hover:text-indigo-900">
                                    {{ $ticket->title }}
                                </a>
        ...
        <div class="bg-white rounded-lg shadow">
            <div class="px-6 py-4 border-b border-gray-200">
                <h3 class="text-lg font-medium text-gray-900">Tickets Disponibles ({{ $unassignedTickets->count() }})</h3>
            </div>
                ...
                <div class="divide-y divide-gray-200">
                    @foreach($unassignedTickets as $ticket)
                        <div class="px-6 py-4">
                            ...
                            <p class="text-sm text-gray-600 mb-2">{{ Str::limit($ticket->description, 80) }}</p>
                            <div class="flex items-center justify-between">
                                <span class="text-xs text-gray-500">{{ $ticket->user->name }} - {{ $ticket->created_at->diffForHumans() }}</span>
